...
class Ghost(Widget):
    ...
    def move(self, randomly=True):
        ...
        if self.pos == last_pos:
            if (self.strat[1] == [] or randomly) and (
                (self.pos[0], self.pos[1]) in bound
            ):
                self.direction()
                self.move()
    ...
    def strategy(self):
        ...
        try:
            ...
            if (bound[self.strat[1][0]][1] - self.pos[1]) != 0:
                self.velocity = [
                    0,
                    (bound[self.strat[1][0]][1] - self.pos[1])
                    / abs(bound[self.strat[1][0]][1] - self.pos[1]),
                ]

            if (bound[self.strat[1][0]][0] - self.pos[0]) != 0:
                self.velocity = [
                    (bound[self.strat[1][0]][0] - self.pos[0])
                    / abs(bound[self.strat[1][0]][0] - self.pos[0]),
                    0,
                ]
            self.move(False)
        except:
